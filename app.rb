...
def revision_url(digest)
  base = ENV['REVISION_URL_BASE']
  if base
    base + digest
  end
end
...
class Server < Sinatra::Base
  post '/notify' do
    if ENV['NEWRELIC_API_KEY'] and (params[:new_relic_app_id] || ENV['NEWRELIC_APP_ID'])
      nn = NewrelicNotification.new(api_key: ENV['NEWRELIC_API_KEY'],
                                    app_id: params[:new_relic_app_id] || ENV['NEWRELIC_APP_ID'],
                                    user: params[:user],
    ...
    if ENV["SLACK_HOOK_URL"]
      sn = SlackNotification.new(hook_url: ENV["SLACK_HOOK_URL"],
                                 app: params[:app],
                                 revision: params[:head_long],
                                 revision_url: revision_url(params[:head_long]),
                                 git_log: params[:git_log])
      sn.notify!
    end
